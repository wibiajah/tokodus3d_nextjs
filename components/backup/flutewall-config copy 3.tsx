...
export const flutePresets = {
  A: { key: "A", val: 4.8, multiplier: 0.16, la: 10, lb: 8, z: 2.4, ax: 0.5, ka:0.3, kb:0.5, kc:1 },
  B: { key: "B", val: 3.2, multiplier: 0.25, la: 10, lb: 8, z: 6, ax: 0.5, ka:0.2, kb:0.1, kc:1 },
  C: { key: "C", val: 4, multiplier: 0.2, la: 10, lb: 8, z: 6, ax: 0.5, ka:0.2, kb:1, kc:1 },
  E: { key: "E", val: 1.6, multiplier: 0.5, la: 10, lb: 8, z: 11, ax: 0.5, ka:0.2, kb:1, kc:1 },
  F: { key: "F", val: 0.8, multiplier: 1, la: 10, lb: 8, z: 17, ax: 0.5, ka:0.2, kb:0.2, kc:1 },
} as const;//cek garis lurus dan gelomabng apakah sudah sesuai?
// nilai yang mempengaruhi a, aa, b, ab, c, ac
// adalah (val, multiplier)=0.8, z, ax, w
...
export function useFluteWallConfig(fluteKey: string, wall: string) {
  return useMemo(() => {
    ...
    const omega = flute.z * flute.multiplier;
    const ya = Math.abs((Math.sin(omega) * flute.ka) * flute.ax);
    const yb = 1-Math.abs(((Math.sin(omega + Math.PI/2) * flute.kb) * flute.ax));
    const yc = (Math.sin(omega + Math.PI) * flute.kc) * flute.ax;
    // console.log("DEBUG: ya,yb,yc", ya, yb, yc);
    // const ya = z*ax;
    // const yb = z*ax;
    // const yc = z*ax;
    const aa = 60;
    const ab = aa * 1.7;
    const ac = aa * 2.6;
    const a = aa * (0.2 + 0.7 * ya);
    const b = aa + (ab - aa) * (0.2 + 0.7 * yb);
    const c = ab + (ac - ab) * (0.2 + 0.7 * yc);

    const points = { fwa: a, fwb: b, fwc: c, aa, ab, ac };
    const ry = wallBase.ry * flute.multiplier;
    const rx = wallBase.rx * flute.multiplier;
